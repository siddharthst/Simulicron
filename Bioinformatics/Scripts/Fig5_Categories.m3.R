...
filename<-paste("ResultsDmel.",names(sra[n]),"/AllTEs/Dmel.",sra[n],".250.3.TE.summary.txt.2.txt",sep='')
...
write.table(tab[,-c(11,27,28,29)],paste("ResultsDmel/Table.",sra[n],".All.CpNb.m3.txt",sep=""), sep='\t',row.names=FALSE,col.names=TRUE, quote=FALSE)
...
write.table(tabtot,"ResultsDmel/Table.All.CpNb.m3.txt", sep='\t',row.names=TRUE,col.names=TRUE, quote=FALSE)

########### GLM stats

library(multcomp)
#tt <- read.table("Table.All.CpNb.txt", sep="\t", header=TRUE)
tt <- read.table("ResultsDmel/Table.All.CpNb.m3.txt", sep="\t", header=TRUE)
...
sink("ResultsDmel/Stat.All.CpNb.m3.txt")
...
write.table(tabFinal,"ResultsDmel/TableFinal.All.Category.m3.txt", sep='\t',row.names=FALSE,col.names=TRUE, quote=FALSE)
...
pdf("ResultsDmel/Figure5.m3.pdf", width=10,height=5)
par(mar=c(7,18,5,2))
