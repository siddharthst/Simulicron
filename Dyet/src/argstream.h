/***************************************************************************
                          argstream.h  -  description
                             -------------------
    version              : March 28 2003
    copyright            : (C) 2003 by Xavier Décoret
...
parameter(const char* l,
	  ...
	  bool mandatory = true)
{
  return ValueHolder<T>(0,l,b,desc,mandatory);
}
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
{
public:
  inline OptionHolder(const char s,
	       const char* l,
	       bool& b,
	       const char* desc);
  inline friend argstream& operator>>(argstream& s,const OptionHolder& v);
...
protected:
  inline OptionHolder(const char s,
	       const char* l,
	       const char* desc);  
  friend OptionHolder help(const char s='h',
			   const char* l="help",
...
option(const char* l,
       ...
       const char* desc="")
{
  return OptionHolder(0,l,b,desc);
}
inline OptionHolder
help(const char s,
     const char* l,
...
//************************************************************
template<class T>
ValueHolder<T>::ValueHolder(const char s,
			    const char* l,
			    T& v,
			    const char* desc,
			    bool mandatory)
...
//************************************************************
OptionHolder::OptionHolder(const char s,
			   const char* l,
			   bool& b,
			   const char* desc)
  : shortName_(1,s),
...
//************************************************************
inline
argstream::argstream(int argc,char** argv,bool debug)
  : debug_(debug),
    progName_(argv[0]),
    minusActive_(true),
    isOk_(true)
